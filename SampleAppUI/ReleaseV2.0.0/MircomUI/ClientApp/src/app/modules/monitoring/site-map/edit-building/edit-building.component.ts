...
export class EditBuildingComponent implements OnInit, OnDestroy {
      ...
      swal({
        ...
        if (result.value) {
          if (!this.selectedBuildingSerialNumber) {
            alert('Please select any building which you want to remove');
          } else {
            this.buildingsDetails.forEach(buildingObj => {
  ...
  onBuildingSubmitted(ev) {
    ...
    if (!this.buildingData.buildingName) {
      this.errorMessageOnBuildingPopUp += 'Name is required. \n';
    }

    if (this.buildingData.buildingSlotNo < 1 || !this.buildingData.buildingSlotNo) {
      this.errorMessageOnBuildingPopUp += 'Please enter number of floors (atleast one). \n';
    }

    if (this.buildingData.buildingSlotNo > 300) {
      this.errorMessageOnBuildingPopUp += 'You can create maximum 300 floors.';
    }
  ...
  onfloorActionClick(ev, actionPerformed, selectedFloor, buildingSerialNumber) {
    ...
    switch (actionPerformed) {
      ...
      case FloorLabelActions[FloorLabelActions.Edit]: {
        ...
        this.router.navigate(['./monitoring/site-map/edit-map', { siteId: this.siteId, mapGuid: selectedFloor.mapGuid }]);
        break;
      }
      /**
      ...
      case FloorLabelActions[FloorLabelActions['Insert Above']]: {
        ...
        this.isCut = false;
        this.isCopied = false;
        if (this.floorsData.filter(z => z.action !== ActionType.Delete).length >= 300) {
          alert('You can create maximum 300 floors.');
        } else {
