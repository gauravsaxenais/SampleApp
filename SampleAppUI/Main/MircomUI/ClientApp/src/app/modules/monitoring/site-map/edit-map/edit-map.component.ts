...
export class EditMapComponent implements OnInit, OnDestroy {
        ...
        this.addAxisSiteMapData.forEach(axisSiteMapObj => {
          if (siteMapObj.type === MapItemType[MapItemType.Building]) {
            if (axisSiteMapObj.itemGuid === siteMapObj.itemGuid) {
              siteMapObj.x = axisSiteMapObj.x;
              siteMapObj.y = axisSiteMapObj.y;
            }
          } else {
  ...
  drag(ev, sourceItem) {
    ...
    if (sourceItem.type === MapItemType[MapItemType.Building]) {
      ...
      const index = this.buildingData.map(function(e) { return e.itemGuid; }).indexOf(sourceItem.itemGuid);
      if (index > -1) {
        this.buildingData.splice(index, 1);
      }
    } else if (sourceItem.type === MapItemType[MapItemType.AccessPoint]) {
      const index = this.accessPointData.map(function(e) { return e.itemId + e.panelGuid; }).indexOf(sourceItem.itemId + sourceItem.panelGuid);
      if (index > -1) {
        this.accessPointData.splice(index, 1);
      }
    } else if (sourceItem.type === MapItemType[MapItemType.Input]) {
      const index = this.inputData.map(function(e) { return e.itemId + e.panelGuid; }).indexOf(sourceItem.itemId + sourceItem.panelGuid);
      if (index > -1) {
        this.inputData.splice(index, 1);
      }
    } else if (sourceItem.type === MapItemType[MapItemType.Camera]) {
      const index = this.cameraData.map(function(e) { return e.itemId + e.panelGuid; }).indexOf(sourceItem.itemId + sourceItem.panelGuid);
      if (index > -1) {
  ...
  onRemoveBuildingClick(ev) {
    ...
    if (this.selectedDeleteGuid === undefined || this.selectedDeleteGuid < 0) {
      alert('Please select any item which you want to remove');
    } else {
      swal({
