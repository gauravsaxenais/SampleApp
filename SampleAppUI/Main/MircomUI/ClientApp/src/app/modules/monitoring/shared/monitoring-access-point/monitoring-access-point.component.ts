...
export class MonitoringAccessPointComponent implements OnInit, OnDestroy {
   ...
   * @param command
   */
  onCommandClick(event, objectId, command, mapGuid, channelGuid) {
            ...
            (response: any) => {
              if (response.success && response.message.messageCode === 200 && response.data != null) {
                ...
                this.accessPointData.forEach(accessPointObj => {
                   ...
                   */
                  const data = this.accessPointStatusData.find(ob => parseInt(ob.objectId.split('_')[1], 10) === accessPointObj.accesspointId);
                  /**
